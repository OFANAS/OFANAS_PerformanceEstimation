...
class EvolutionFinder:
    ...
    def random_sample(self):
        ...
        while True:
            sample = self.arch_manager.random_sample_compofa()
    ...
    def create_latency_dataset(self, search_space, num_of_samples):
        """Creates a dataset of sample child architectures and corresponding latencies and accuracies"""

        for i in range(num_of_samples):
            sample = ''
            if search_space == 'compofa':
                sample = self.arch_manager.random_sample_compofa()
            elif search_space == 'ofa':
                sample = self.arch_manager.random_sample_ofa()
            efficiency = self.efficiency_predictor.predict_efficiency(sample)
            self.latency_dataset['child_arch'].append(sample)
            self.latency_dataset['latency'].append(efficiency)
            print('completed {}/{} samples'.format(i,num_of_samples))

        # Dont need accuracy predictions
        # accs_dataset = self.accuracy_predictor.predict_accuracy(self.latency_dataset['child_arch'])
        # for i in accs_dataset:
        #     self.latency_dataset['accuracy'].append(i.item())

        return self.latency_dataset
