...
class LatencyPredictor(nn.Module):
    ...
    def predict_efficiency(self, spec):
        latency = self.latency_tables.get((tuple(spec['e']), tuple(spec['d']), tuple(spec['ks'])))
        if latency is not None:
            return latency
        encoded = latency_encoding(spec)
        latency = self.forward(encoded).item()
...
def test_model(test_dataset, model_path, finetune=False):
    ...
    if finetune:
        key_names = ["model.0.weight", "model.0.bias", "model.2.weight", "model.2.bias", "model.4.weight",
                     "model.4.bias", "model.6.weight", "model.6.bias"]
...
def main(given_train_size):
    ...
    dataset_path = 'datasets/Tesla_P40_GPU/Tesla_P40_GPU_ofa.csv'
    ...
    model_path = '../checkpoints/latency_prediction_model/checkpoint_Tesla_P40_GPU.pt'
    ...
    test_loss_list = test_model(test_dataset, model_path)
