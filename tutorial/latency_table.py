...
class LatencyTable:
	def __init__(self, device='note10', resolutions=(160, 176, 192, 208, 224),
		...
		if self.use_latency_table:
			...
			# from run import MODEL_DIR
			model_dir = '.torch/ofa_nets/ofa_mbv3_d234_e346_k357_w1.0'
			self.ofa_network = ofa_net(network, model_dir=model_dir, pretrained=True).cuda()
	...
	def predict_efficiency(self, spec: dict):
		if self.use_latency_table:
			...
			net_info = get_net_info(subnet, input_shape=( 3, spec['r'][0], spec['r'][0]),
									measure_latency='gpu64', print_info=False)
			latency = net_info['gpu64 latency']['val']
